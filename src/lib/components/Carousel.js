...
export class Carousel extends React.Component {
    ...
    * constructor : コンストラクタ
    ...
    constructor(props) {
      ...
      this._mouseDownPos = {x:0, y:0, src:{x:0, y:0}};
    ...
    * _onTouchStart
    ...
    _onTouchStart(evn)
    {
      ...
      this._mouseDownPos.src.y = touch.clientY;
    ...
    * _onTouchMove
    ...
    _onTouchMove(evn)
    {
      ...
      this._diffX = this._mouseDownPos.src.x - evn.touches[0].clientX;
      

      //ドラッグ開始しきい値
      if(Math.abs(this._diffX) < this.props.minimumTouchDrag)
        return;

      //差がプラスかマイナスで加算か減算に切り替える
      const diffMinimumTouchDrag = this._diffX > 0 ? this.props.minimumTouchDrag : this.props.minimumTouchDrag * -1;

      //ドラッグ移動量計算
      let posX = this._mouseDownPos.x - evn.touches[0].clientX - diffMinimumTouchDrag;

      this._currentPos.x = parseInt(posX) * -1;
    ...
    * アニメーション処理
    ...
    _animstep(timestamp) {
      ...
      //開始位置と移動量とアニメーション保管値から現在の位置を計算
      this._currentPos.x =  Math.round(this._startPosX + this._movePosX * val);
    ...
    * 現在のスライドインデックスを取得
    ----------------------------------------------- */
    _getCurrentSlideIndex(addX = 0)
    {

      let index = Math.round((this._currentPos.x + addX) / this._getWidth() * -1);
    ...
    * _onTouchEnd
    ...
    _onTouchEnd(evn)
    {
      const speedX = this._diffX / (new Date().getTime() - this._touchStartTime);

      //現在のスライド位置
      const slideIndex = this._getCurrentSlideIndex(-200 * speedX);
    ...
    * スライドのインデックス指定
    ...
    goToSlide(slideIndex)
    {
      ...
      if(
        slideIndex < 0 ||
        this.props.children.length <= slideIndex
      )
        return;

      //現在位置を開始位置に置き換える
      this._startPosX = this._currentPos.x;

      //移動位置
      this._movePosX = slideIndex * this._getWidth() * -1;

      //移動量を計算
      this._movePosX -= this._startPosX;
